...
class Volumes:
    ...
    def createDisk_vstor(self, storageManageId: str, pageSize: str, 
        ...
        :param pageSize: 磁盘页面大小 4KB/8KB/16KB/32KB
        ...
        print("Volumes createDisk payload:", payload)
        response = self.audit.session.post(url, json=payload, verify=False)
        print("Volumes createDisk response:", response.json())
    ...
    def deleteDisk(self, diskId: list) -> dict:
        ...
        payload = {"ids":diskId}
        print("Volumes deleteDisk payload:", payload)
        response = self.audit.session.post(url, json=payload, verify=False)
        print("Volumes deleteDisk response:", response.json())
        return response.json()
