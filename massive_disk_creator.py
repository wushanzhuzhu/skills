...
class MassiveDiskCreator:
    ...
    def create_single_batch(self, batch_num: int, total_disks: int = 100, disk_size: int = 10) -> dict:
        ...
        print(f"🚀 开始创建第 {batch_num}/10 批磁盘")
        print(f"📁 磁盘编号: {start_index:04d} - {end_index:04d}")
        print(f"💾 每个大小: {disk_size}GB")
        print(f"🎯 本批总数: {total_disks}个")
        print("=" * 60)
        ...
        for i in range(start_index, end_index + 1):
            ...
            try:
                ...
                # 创建磁盘
                print(f"🔧 创建配置: 大小={disk_size}GB, IOPS={config_result['config']['iops']}, 带宽={config_result['config']['bandwidth']}MB/s")
        ...
        # 显示批次结果
        print(f"\n" + "=" * 70)
        print(f"📊 第 {batch_num} 批次创建结果汇总")
        print("=" * 70)
        print(f"✅ 成功创建: {success_count}/{total_disks}")
        print(f"❌ 创建失败: {failed_count}/{total_disks}")
        print(f"📈 成功率: {success_count/total_disks*100:.1f}%")
        print(f"💾 成功容量: {success_count * disk_size}GB")
    ...
    def create_all_batches(self, total_batches: int = 10, disks_per_batch: int = 100, disk_size: int = 10):
        ...
        print(f"📁 每批磁盘: {disks_per_batch}个")
        print(f"💾 磁盘大小: {disk_size}GB")
        print(f"📊 总磁盘数: {total_batches * disks_per_batch}个")
        print(f"💾 总容量: {total_batches * disks_per_batch * disk_size}GB")
        print(f"🏷️ 命名规则: batch-001 到 batch-{total_batches * disks_per_batch:04d}")
        print("=" * 70)
        ...
        for batch_num in range(1, total_batches + 1):
            print(f"\n{'='*80}")
            print(f"🚀 开始执行第 {batch_num}/{total_batches} 批次")
            print(f"{'='*80}")
        ...
        print(f"📈 总成功率: {total_success/(total_batches * disks_per_batch)*100:.1f}%")
        print(f"💾 成功总容量: {total_success * disk_size}GB")
