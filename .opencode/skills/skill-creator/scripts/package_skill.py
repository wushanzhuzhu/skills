...
def package_skill(skill_path, output_dir=None):
    ...
    if not skill_path.exists():
        logger.error(f"❌ Error: Skill folder not found: {skill_path}")
        return None

    if not skill_path.is_dir():
        logger.error(f"❌ Error: Path is not a directory: {skill_path}")
        return None

    # Validate SKILL.md exists
    skill_md = skill_path / "SKILL.md"
    if not skill_md.exists():
        logger.error(f"❌ Error: SKILL.md not found in {skill_path}")
        return None
    ...
    if not valid:
        ...
        logger.info("   Please fix the validation errors before packaging.")
        return None
