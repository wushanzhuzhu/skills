    ...
    def create_final_batch(self, start_num: int, end_num: int = 1000) -> dict:
        ...
        for i in range(start_num, end_num + 1):
            ...
            try:
                ...
                # 创建磁盘
                logger.info(f"🔧 创建配置: 大小=10GB, IOPS={config['iops']}, 带宽={config['bandwidth']}MB/s")
                ...
                if isinstance(result, dict):
                    if 'data' in result and result['data'] and len(result['data']) > 0:
                        ...
                        logger.warning(f"⚠️ {disk_name} 已存在，跳过")
                        ...
                        logger.error(f"❌ {disk_name} 创建失败: {result.get('errorMessage', '未知错误')}")
                        failed_count += 1
                        results.append({
                            'disk_name': disk_name,
                            'disk_num': i,
                ...
                if progress % 50 == 0:
                    ...
                    logger.info(f"💾 已创建容量: {success_count * 10}GB")
        ...
        logger.info(f"💾 成功总容量: {success_count * 10}GB")
...
def main():
    """命令行界面"""
    logger.info("🔥 最终磁盘创建器")
    logger.info("从batch-243到batch-1000创建剩余的758个10GB磁盘")
    logger.info("=" * 60)
