...
def check_disk_info():
    logger.info("session serverip:", session['server_ip'])
    vmname = request.form['instance_name']
    logger.info("from form vmname:", vmname)
...
def upload_image():
    if request.method == 'GET':
        server_ip = request.args.get('serverip')
        session['server_ip'] = server_ip
        logger.info("session serverip:", server_ip)
    ...
    if request.method == 'POST':
        ...
        logger.info("ip:{}.架构is_X86{},zoneID{},storageType{},storageManageId({}".format(session['server_ip'], host.is_X86(),data["zoneId"],data["storageType"],data["storageManageId"]))
        if host.is_X86():
...
def import_license():
    ...
    if request.method == 'POST':
        ...
        logger.info("app  licensetxt:",licensetxt)
